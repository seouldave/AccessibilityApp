...
class Geoserver:
	...
	def apply_style(self):
		...
		None
		"""
		resource = 'layers/dissertation:time-cost-raster'
		payload = '<layer><defaultStyle><name>time-cost-style</name></defaultStyle></layer>'
		headers = {'Content-type': 'text/xml'}
		request_url = urljoin(self.api_entry, resource)
		resp = requests.put(request_url, auth=self.credential, data=payload, headers=headers)
...
#############################################################################################################################
	#Upload style to geoserver ###################CHECK ABOVE upload_style()
	def upload_style():
		################################CHECK IF THIS FUNCTION CAN BE REMOVED
		resource = 'styles'
...
#############################################################################################################################
	...
	def make_sld(self, cost_distance_threshold):
		...
		upload_style(filename, sld_name)

	def raster_to_geoserver(self, num_hours):
		"""Main function to call functions to create files and upload to Geoserver.

		Arguments:
		num_hours -> Threshold travel-time
		...
		upload_raster_geoserver('opt/geoserver/data_dir/produced_rasters/time-cost-raster.tif.zip') #Upload raster to Geoserver
		make_sld(num_hours) #Create SLD file and upload to Geoserver
