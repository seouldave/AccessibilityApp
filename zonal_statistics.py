...
class Zonal_statistics:
	...
	def shp_to_postGIS(self, postgis_table, shp_bin, country_chosen, population_chosen):
		...
		cursor.execute("CREATE INDEX levelIndex ON {0}(value)".format(postgis_table)) #Create index on threshold zones
		cursor.execute("SELECT AddGeometryColumn('{0}', ".format(postgis_table) +
									"'geom', 4326, 'POLYGON', 2)")
		cursor.execute("CREATE INDEX geomIndex ON {0} ".format(postgis_table) + #Create index on geometry
						"USING GIST (geom)")
		connection.commit()
		...
		for pol in [0,1]:
			...
			for i in range(layer.GetFeatureCount()):
				...
				if feature.GetField('Value') == 1:
					...
					cursor.execute("INSERT INTO {0} (value, geom) ".format(postgis_table) + #Insert table into database
		...
		cursor.execute("UPDATE {0} SET geom = ST_MAKEVALID(geom) WHERE NOT ST_IsValid(geom)".format(postgis_table)) #Fix self-intersecting geometries
	...
	def get_zonal_stats(self, country_chosen):
		...
		connection.set_isolation_level(0)
		cursor = connection.cursor()
		cursor.execute("SELECT row_to_json(zonal_stats_{0}) as json FROM public.zonal_stats_{0}".format(country_chosen)) #Query to select table and convert to JSON
